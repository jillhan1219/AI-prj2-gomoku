...
class DQNNet(nn.Module):
    ...
    def predict(self, state: State) -> Variable:
        ...
        min_act_value = torch.min(act_value).item()
        # Set every illegal moves to have value 0
        #print(f'act vaue: {act_value.shape}')
        #print(f'state vaue: {state_var.view(1,225).shape}')
        indeces = state_var.view(1,225) > -1
        #act_value[state_var.view(-1)>-1] = min_act_value
        act_value[indeces] = min_act_value
